<!-- A draggable table inside a modal, used to update sort indexes -->
<template>
  <n-modal :loading="loading" v-model:show="show" preset="dialog" :title="title" :positive-text="t('button.submit')"
    ...
    <n-data-table ref="tableRef" :columns="columns" :data="data" :bordered="false"
      :row-key="(row: any) => row[keyField]" max-height="500px" />
  </n-modal>
</template>
...
function hookSortable() {
  ...
  const tbody: HTMLElement | null = el.querySelector('.n-data-table-tbody');
  ...
  Sortable.create(tbody, {
    onEnd(evt: any) {
      const oldElem = data.value[evt.oldIndex!]!!;
      data.value.splice(evt.oldIndex!, 1);
      data.value.splice(evt.newIndex!, 0, oldElem);
    }
  });
}

function handlePositiveClick() {
  emit('select', data.value.map(v => (v as any)[props.keyField]));
  show.value = false;
}
